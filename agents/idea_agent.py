...
Runs every Sunday night via cron. Orchestrates:
  1. Scrape YouTube for trending content in niche
  2. Generate 10 video ideas with Claude
  3. Write ideas to Google Sheet
  4. Email user with ideas + sheet link + approval instructions

Usage:
    python3 agents/idea_agent.py
    python3 agents/idea_agent.py --niche "Finance"  # override niche
"""
...
def build_ideas_email(ideas_json_path, sheet_url, niche):
    ...
    lines = [
        f"Your weekly YouTube video ideas for '{niche}' are ready!",
        "",
    ...
    for idea in ideas:
        ...
        lines.append(f"  {idea['id']}. \"{idea['title']}\" {potential_tag}")
        lines.append(f"     Hook: {idea.get('hook', '')[:100]}")
        lines.append("")
    ...
    lines += [
        ...
        "",
        "I'll generate scripts, voiceovers, and videos for approved ideas",
        "and send you links to review before publishing.",
        "",
        "—YouTube Automation",
    ]
...
def main():
    ...
    print(f"[idea_agent] Starting weekly idea generation for '{niche}' (week {week_str})")
    ...
    print("[1/5] Scraping YouTube trending videos...")
    try:
        ...
        log_error(f"YouTube scrape failed: {e}")
        ...
        if not args.dry_run:
            try:
                run_tool("send_email.py", [
                    ...
                    "--body", f"The weekly idea generation encountered an error:\n\n{e}\n\nPlease check the logs at .tmp/cron.log",
                ])
            except Exception:
                pass
        sys.exit(1)
    ...
    print("[2/5] Generating 10 video ideas with Claude (viral researcher mode)...")
    try:
        generate_args = [
            "--scraped-file", scraped_path,
            "--niche", niche,
            "--count", "10",
    ...
    email_body = build_ideas_email(ideas_json, sheet_url, niche)
