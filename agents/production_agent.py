...
def produce_video(idea_id, video_key, ideas_path):
    ...
    if not script_valid:
        ...
        try:
            run_tool("generate_retention_script.py", [
                "--idea-id", str(idea_id),
    ...
    if footage_valid:
        ...
        print(f"  [C] Fetching stock footage...")
        try:
            run_tool("fetch_pexels_footage.py", [
                "--script-file", script_path,
    ...
    niche_env = os.getenv("NICHE", "Self Development")
...
def build_review_email(produced_videos, niche):
    """Build the video review email body."""
    lines = [
        f"Your {len(produced_videos)} video(s) for '{niche}' are ready for review!",
        "",
...
def main():
    ...
    email_body = build_review_email(produced, niche)
