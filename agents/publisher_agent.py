...
def get_publish_slots(n: int) -> list:
    """Return n UTC ISO timestamps for Mon/Wed/Fri at 9am IST (03:30 UTC) starting next week."""
    # IST is UTC+5:30, so 9am IST = 03:30 UTC
    PUBLISH_HOUR_UTC = 3
...
def main():
    ...
    print(f"[publisher_agent] Scheduling {len(approved_video_ids)} video(s) for Mon/Wed/Fri at 9am IST...")
    ...
    if published:
        lines = [
            f"{len(published)} video(s) scheduled to publish this week at 9am IST!",
            "",
        ]
        for vid in published:
            ...
            lines.append(f"    {vid['day']} {ist_dt.strftime('%d %b')} at 9:00am IST")
