...
def process_segment_clip(clip_path, needed_duration):
    """Load, resize, and trim/loop a stock footage clip."""
...
def main():
    ...
    for i, (seg, duration) in enumerate(zip(valid_segments, segment_durations)):
        ...
        if not clip_paths:
            ...
            vid_clip = ColorClip((TARGET_WIDTH, TARGET_HEIGHT), color=(0, 0, 0), duration=duration)
        else:
            try:
                vid_clip = assemble_segment_with_cuts(clip_paths, duration, seg_id)
            except Exception as e:
    ...
    if args.captions_file and os.path.exists(args.captions_file):
        ...
        for chunk in chunks:
            try:
                cap_clip = make_caption_chunk_clip(chunk["text"], chunk["start"], chunk["end"])
                overlay_clips.append(cap_clip)
                cap_ok += 1
            except Exception as e:
                print(f"  WARNING: Caption chunk failed '{chunk['text']}': {e}", file=sys.stderr)
        print(f"  {cap_ok}/{len(chunks)} caption chunks added.", file=sys.stderr)
