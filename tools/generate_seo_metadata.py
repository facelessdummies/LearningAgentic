...
def build_prompt(script, niche, strategy=None):
...
2. DESCRIPTION (150-250 words):
   ...
   - End with a CTA to subscribe and a note about the channel's niche content
...
7. SEARCH PHRASES: 5 exact phrases (how people search) that this video should rank for. Be specific — not "self development" but "morning routine to be more productive".
...
def main():
    ...
    prompt = build_prompt(script, niche, strategy)
